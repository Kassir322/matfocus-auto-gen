...
def run_mode(
    ...
    if missing_refs:
        ...
        for side, card_num, card_name, expected in missing_refs:
            print(f"  - {side} карточки {card_num} ({card_name})")
        print("[INFO] Продолжаем генерацию без отсутствующих референсов")
    ...
    try:
        ...
        write_log_line(
            log_file,
            f"[PLAN] Режим: multiformat_with_refs. Формат лицо: {face_ratio}, оборот: {back_ratio}. Карточек: {info['cards_count']}, пар: {info['pairs_count']}, изображений: {info['images_planned']}",
        )
        if not missing_refs:
            write_log_line(log_file, f"[CHECK] Референсы: {found_refs}/{total_refs} найдено")
        if prompts_file:
        ...
        for line in stats.start_summary_lines(
            [
                "Режим: multiformat_with_refs (browser)",
                ...
                f"Референсы найдены: {found_refs}, отсутствуют: {len(missing_refs)}",
            ]
        ):
            print(line)
        ...
        for idx, task in enumerate(tasks):
            ...
            result = _generate_single_side_with_ref(task, aspect_ratio, coordinates, relative_movements, settings, log_file)
            duration_seconds = time.monotonic() - attempt_started
            ok, reason = generation_stats.normalize_attempt_result(task, result)
            label = f"карточка {card_number} {side} пара {pair_number}" + (" with_ref" if task.get("_with_reference") else "")
