"""
Стандартный режим генерации через Gemini API (без браузера).
...
def load_tasks_from_file(path: str) -> list[dict]:
    """
    Загрузка задач из файла промптов стандартного формата.
    ...
    Returns:
        Список задач (dict): card_number, generation_number, prompt_text
    """
    return parse_standard_prompts(path)
...
def get_plan_info(tasks: list[dict]) -> dict:
    ...
    Returns:
        dict: cards_count, generations_count, images_planned
    """
    return get_plan_info_standard(tasks)
...
def _generate_single_image_api(
    ...
    """
    Генерация одного изображения через API.
    
    Args:
        task: задача с полями card_number, generation_number, prompt_text
        client: экземпляр genai.Client
        settings: настройки (API_MODEL, API_IMAGE_SIZE, API_TIMEOUT)
    ...
    write_log_line(log_file, f"[GEN] Генерация через API: карточка {card_number} генерация {gen_num}")
...
def run_mode(
    ...
    """
    Выполнение генерации для стандартного режима через API.
    
    Args:
        tasks: список задач из load_tasks_from_file
        settings: настройки (API_KEY, API_MODEL, START_FROM_CARD, END_CARD и т.д.)
    ...
    try:
        ...
        plan_msg = (
            f"[PLAN] Режим: standard (API). Карточек: {info['cards_count']}, "
