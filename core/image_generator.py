    ...
    def process_card(self, card_number, card_name, prompts_for_card, generations_per_card, check_image_enabled, generation_wait, stop_event):
        ...
        try:
            ...
            for gen_num in range(1, generations_per_card + 1):
                ...
                # Проверяем, является ли название дефолтным (чтобы избежать дублирования в имени файла)
                default_name = f"Карточка {card_number}"
                if card_name == default_name:
                    # Если название дефолтное, не включаем его в имя файла
                    filename = f"Карточка_{card_number}_генерация_{gen_num}.png"
                else:
                    # Если название пользовательское, включаем его
