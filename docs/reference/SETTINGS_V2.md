...
Образец безопасных настроек хранится в `data/settings.example.json`. При первом
сохранении меню создаёт локальный `data/settings.json`, который не отслеживается
Git. Секреты не записывайте в эти файлы: используйте `GOOGLE_API_KEY`,
`OPENAI_API_KEY` или локальный `.env`.
...
текущем запуске и не изменяют локальный `data/settings.json`.
