...
- Поддерживаются неполные пары: можно указать только `лицо` или только `оборот`.
- Если пара неполная, будет сгенерировано только одно изображение (для имеющейся стороны).

Мини-пример с полными парами:
...
Пример с неполными парами (только лицо):

```
Карточка 2 лицо Кот - Промпт 1: a cute cat, 4k
Карточка 2 лицо Кот - Промпт 2: cat variant, studio
```
...
- **Мультиформатный без референсов (multi_format)**: пары изображений с настраиваемыми соотношениями сторон. Требует координаты `ASPECT_RATIO_SELECTOR`.
...
- **Мультиформатный с референсами (multi_format_with_refs)**: пары изображений с настраиваемыми соотношениями сторон и использованием референсов.
...
2. Настройте координаты (Ctrl+0). Для мультиформатных режимов обязательно `ASPECT_RATIO_SELECTOR`.
...
- Неправильный формат промптов: проверьте, что формат соответствует выбранному режиму генерации.
  - Стандартный режим: `Карточка X - Промпт Y: текст`
  - Мультиформатный режим: `Карточка X лицо Название - Промпт Y: текст` или `Карточка X оборот Название - Промпт Y: текст`
