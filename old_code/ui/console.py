    ...
    def show_current_settings(self, settings_manager):
        ...
        if generation_mode in ['multi_format', 'multi_format_with_refs']:
            ...
            if generation_mode == 'multi_format_with_refs':
                print(f"  Референсы требуются в папке data/images")
        else:
        ...
        if missing_coords:
            print(f"  Не заданы: {', '.join(missing_coords)}")
        else:
        ...
        if available_cards and settings_manager.get('START_FROM_CARD') in available_cards:
            ...
            if generation_mode in ['multi_format', 'multi_format_with_refs']:
                total_pairs = sum(len(pairs) if isinstance(pairs, list) else len(pairs[1]) for card_num, pairs in all_prompts.items() 
                ...
                if generation_mode == 'multi_format_with_refs':
                    ...
                    if not os.path.exists(face_dir) or not os.path.exists(back_dir):
                        print(f"  Папки с референсами не найдены: {face_dir}, {back_dir}")
                    else:
            ...
            print(f"  ВНИМАНИЕ: Стартовая карточка {settings_manager.get('START_FROM_CARD')} не найдена в файле!")
        print("-" * 60)
